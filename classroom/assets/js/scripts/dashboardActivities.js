   ...
   if (!UserManager.getUser().isRegular && Activity.correction > 0) {
    ...
    if(content.startsWith('http')) {  // TODO replace with "if content is LTI"
      if (UserManager.getUser().isRegular && Activity.correction > 0) {
       ...
       let ltiReviewSubmission = $('#lti-review-submission').html('<iframe style="width: 100%; height: 100%;" allowfullscreen="true" frameborder="0" src="https://cabricloud.com/ed/opensteam/player?calculator=false&clmc=' + Activity.commentary + '" allowfullscreen></iframe>');
